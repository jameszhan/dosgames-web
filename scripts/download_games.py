...
root = os.path.dirname(os.path.abspath(os.path.join(inspect.getfile(inspect.currentframe()), os.pardir)))

PREFIX = "https://dos-bin.zczc.cz/"
DESTINATION = os.path.join(root, 'static', 'games', 'bin')
BUF_SIZE = 65536
THREAD_SIZE = 10
...
def download(identifier, url, file):
    try:
        ...
        print(f'{e} while downloading {identifier}')
        urllib.request.urlretrieve(url.replace(PREFIX, "https://dos.zczc.cz/games/"), file)
